...
def load_wikisimpsons_episodes(scrapes_episodes_data_dir: str) -> pd.DataFrame:
    ...
    episode_files = [
        ...
        for f in os.listdir(scrapes_episodes_data_dir) if f.endswith(".json")
        ]
...
def merge_episodes(nohomer_episodes: pd.DataFrame, 
                   wiki_episodes: pd.DataFrame) -> pd.DataFrame:
...
def get_not_merged_episodes(episodes_data: pd.DataFrame) -> list:
    ...
    not_merged = episodes_data.loc[
        episodes_data["_merge"]=='left_only', 
        "episode_name"
    ].tolist()
    return not_merged
...
def compute_showrunner_dummies(episodes_data, min_presence = 10):
    ...
    showrunner_counts = (
        episodes_data
        .explode("show_runners")['show_runners']
        .value_counts()
    )
...
def select_relevant_columns(episodes_data: pd.DataFrame) -> pd.DataFrame:
    ...
    cols = ['episode_name', 'main_image_url', 'season_nr','episode_number', 
...
def build_episodes():
    ...
    nohomer_episodes["EP_KEY"] = nohomer_episodes["episode_name"].apply(create_episode_key)
    wikisimpsons_episodes["EP_KEY"] = wikisimpsons_episodes["episode_name"].apply(create_episode_key)
    ...
    if not_merged_episodes:
        print(f"Warning: {len(not_merged_episodes)} no-homer episodes could not be merged")
